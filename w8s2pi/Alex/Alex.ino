...
//now right 680 / 5
//now left 765 / 5
#define COUNTS_PER_REV      145 // new
#define COUNTS_PER_REV_R      136 // new
#define COUNTS_PER_REV_L      153 // new
...
volatile unsigned long forwardRDist;
volatile unsigned long forwardLDist;
volatile unsigned long reverseDist;

unsigned long deltaDist;
unsigned long newRDist;
unsigned long newLDist;
...
void sendStatus()
{
    ...
    statusPacket.params[8] = forwardLDist;
...
void leftISR()
{
    ...
    if (dir == FORWARD) {
        leftForwardTicks++;
        forwardLDist = (unsigned long) ((float) leftForwardTicks / COUNTS_PER_REV_L * WHEEL_CIRC); // new
...
void rightISR()
{
    if (dir == FORWARD) {
        rightForwardTicks++;
        forwardRDist = (unsigned long) ((float) rightForwardTicks / COUNTS_PER_REV_R * WHEEL_CIRC); // new
    } else if (dir == BACKWARD) {
        rightReverseTicks++;
    } else if (dir == LEFT) {
...
void forward(float dist, float speed)
{
    ...
    newLDist = forwardLDist + deltaDist; // new
    newRDist = forwardRDist + deltaDist; // new
...
void reverse(float dist, float speed)
{
    ...
    newLDist = reverseDist + deltaDist;  //new
...
void loop() {
    ...
    if (deltaDist > 0) {
        if (dir == FORWARD) {
            if (forwardLDist >= newLDist || forwardRDist >= newRDist) {
                deltaDist = 0;
                newLDist = 0; // new
                newRDist = 0; // new
                stop();
